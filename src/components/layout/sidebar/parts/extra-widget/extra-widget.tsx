...
export const ExtraWidget: FC<ExtraWidgetProps> = async ({ settings }) => {
   const t = await getServerTranslations();
   const tSidebar = await getServerTranslations('sidebar');
      ...
      <>
         <div className="mt-4 hidden w-full rounded-[10px] bg-card p-8 lg:block">
            <div className="flex items-center justify-center gap-2 rounded-[10px] bg-accent py-4 font-bold">
               ...
               <h3 className="text-accent-foreground">{tSidebar('top-donator')}</h3>
            </div>
            <div className="my-4 flex items-start justify-center">
               ...
               <div className="mt-4">
                  <h3 className="text-xl font-bold text-primary">
                     {username || tSidebar('no-top-donor')}
                  </h3>
                  <p className="text-sm text-muted-foreground">
                     {username
                        ? tSidebar('top-donator-hint')
                        : tSidebar('no-top-donor-description')}
                  </p>
               </div>
            </div>
